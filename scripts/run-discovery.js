...
function main() {
  ...
  // DISCOVERY_DETERMINISTIC_AUTH
  const normalizedScenarioArgument = String(
    scenarioArgument
  ).replace(/^scenarios[\\/]/i, '');

  const directScenarioPath = path.resolve(
    projectRoot,
    scenarioArgument
  );

  const absoluteScenarioPath = fs.existsSync(
    directScenarioPath
  )
    ? directScenarioPath
    : path.resolve(
  ...
  if (!fs.existsSync(absoluteScenarioPath)) {
    throw new Error(
      `Scenario file was not found: ${absoluteScenarioPath}`
    );
  }

  const scenarioText = fs.readFileSync(
  ...
  if (
    ...
    const authScriptPath = path.join(
      projectRoot,
      'scripts',
      'prepare-auth-state.js'
    );

    if (!fs.existsSync(authScriptPath)) {
      throw new Error(
        `Deterministic authentication script is missing: ${authScriptPath}`
      );
    }

    console.log('');
    console.log('🔐 Preparing deterministic authenticated session...');
    ...
    if (
      ...
      throw new Error(
        authResult.error
          ? `Deterministic authentication failed: ${authResult.error.message}`
          : `Deterministic authentication failed with exit code ${authResult.status}. Discovery was not started.`
      );
    }
    ...
    const storageStatePath = path.join(
      ...
      'artifacts',
      '.auth',
      `woolworths-${targetEnvironment}.json`
    );

    if (!fs.existsSync(storageStatePath)) {
      throw new Error(
        `Authenticated storage state was not created: ${storageStatePath}`
      );
    }
    ...
    if (storageStateStats.size === 0) {
      throw new Error(
        `Authenticated storage state is empty: ${storageStatePath}`
      );
    }

    /*
     * The runtime and its MCP child process inherit this
     * environment variable from run-discovery.js.
     */
    process.env.PLAYWRIGHT_STORAGE_STATE =
      storageStatePath;

    console.log(
      `✅ Authenticated storage state ready: ${storageStatePath}`
    );
  }
  ...
  const runtimeScenarioArgument = scenarioArgument
    .replace(/\\/g, '/')
    .replace(/^\.\//, '')
    .replace(/^scenarios\//, '');

  const scenarioBase = path.basename(
    runtimeScenarioArgument,
    path.extname(runtimeScenarioArgument)
  );
