...
async function judgeChatbotResponse({
    ...
    try {
      ...
      return {
        ...
        passed:
          ...
          score >= minScore &&
          judgement.safe !== false &&
          judgement.transitionValid !== false,
        judgeMode:
          attempt === 1
            ? 'llm'
            : 'llm_retry',
      ...
      console.error(
        `[LLM judge] Invalid JSON on attempt ${attempt}/${maxAttempts}: ${
          compact(
            lastRawResponse,
            300
          ) ||
  ...
  return {
    ...
    missing: [
      'Valid semantic judge result',
    ],
    issues: [
      lastParseError?.message ||
        'LLM judge returned malformed JSON.',
    ],
    evidence:
      lastRawResponse
        ? [
        ...
        : [],
    summary:
      'Semantic validation could not complete because the LLM judge returned malformed JSON.',
    judgeMode:
